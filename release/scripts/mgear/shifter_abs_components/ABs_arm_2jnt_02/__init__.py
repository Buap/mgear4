...
import mgear.core.curve as cur
...
class Component(MainComponent):
    ...
    def addObjects(self):
        ...
        # limb chain FK ref
        self.limbBonesFK = pri.add2DChain(self.root, self.getName("limbBonesFK%s_jnt"), self.guide.apos[0:3], self.normal, False, self.WIP)

        # limb chain IK ref
        self.limbBonesIK = pri.add2DChain(self.root, self.getName("limbBonesIK%s_jnt"), self.guide.apos[0:3], self.normal, False, self.WIP)

        # IK SYSTEM =======================================================================================================================================
        # ik control
        self.ik_npo = pri.addTransform(self.root, self.getName("ik_npo"), self.guide.atra[-2])
        ...
        # create locators
        self.upperArmIK_loc = pri.addLocator(self.root, self.getName("upperArmIK_loc"), self.guide.atra[0], size=0.2)
        self.wrist_loc = pri.addLocator(self.upperArmIK_loc, self.getName("wrist_loc"), self.guide.atra[-2], size=0.2)
        self.softBlend_loc = pri.addLocator(self.root, self.getName("softBlend_loc"), self.guide.atra[-2], size=0.2)
        ...
        # mid controller
        self.ctrn_loc = pri.addTransform(self.root, self.getName("ctrn_loc"),tra.getTransform(self.mid1_jnt))
        ...
        # twist spline ik curves
        pts = ( tra.getTranslation(self.tws0_loc), tra.getTranslation(self.upperLimbTangentA_ctl), tra.getTranslation(self.upperLimbTangentB_ctl), tra.getTranslation(self.mid_ctl) )
        self.upperArm_crv = cur.addCurve(self.setup, self.getName("upperArmTwist_crv"), pts)
        
        pts = ( tra.getTranslation(self.mid_ctl), tra.getTranslation(self.lowerLimbTangentA_ctl), tra.getTranslation(self.lowerLimbTangentB_ctl), tra.getTranslation(self.eff_loc) )
        self.upperArm_crv = cur.addCurve(self.setup, self.getName("lowerArmTwist_crv"), pts)
    ...
    def addOperators(self):
        ...
        pm.aimConstraint(self.ik_ctl, self.upperArmIK_loc, offset=(0, 0, 0),  weight=1, aimVector=(1, 0, 0),  upVector = (0, 1, 0), worldUpType="objectrotation", worldUpVector=(0, 1, 0), worldUpObject=self.root)
        ...
        scale_mul =  nod.createFloatDiv(1, self.root.scaleX)
        ...
        # Roll SYSTEM =======================================================================================================================================
        pm.aimConstraint(self.limbBones[2], self.tws0_loc, offset=(0, 0, 0),  weight=1, aimVector=(1, 0, 0),  upVector = (0, 0, 1), worldUpType="objectrotation", worldUpVector=(0, 1, 0), worldUpObject=self.root)

        # mid joint and ctrl
        pm.orientConstraint(self.tws0_loc, self.mid1_jnt, mo=True)
        
        pm.orientConstraint(self.mid1_jnt, self.ctrn_loc)
        pm.pointConstraint(self.limbBones[1], self.ctrn_loc)

        # tangents
        pm.aimConstraint(self.upperLimbTangentB_npo, self.upperLimbTangentA_loc, offset=(0,0,0), weight=1, aimVector=(1,0,0), upVector=(0,0,1), worldUpType="objectrotation", worldUpVector=(0,0,1), worldUpObject="limb_C0_mid_ctl")
        pm.aimConstraint(self.lowerLimbTangentA_npo, self.lowerLimbTangentB_loc, offset=(0,0,0), weight=1, aimVector=(-1,0,0), upVector=(0,0,1), worldUpType="objectrotation", worldUpVector=(0,0,1), worldUpObject="limb_C0_mid_ctl")
        pm.pointConstraint(self.eff_loc, self.lowerLimbTangentB_loc)

        pm.aimConstraint(self.upperArmIK_loc, self.tws1A_npo, offset=(0,0,0), weight=1, aimVector=(-1,0,0), upVector=(0,0,1), worldUpType="objectrotation", worldUpVector=(0,0,1), worldUpObject="limb_C0_mid_ctl")
        pm.aimConstraint(self.eff_loc, self.tws1B_npo, offset=(0,0,0), weight=1, aimVector=(1,0,0), upVector=(0,0,1), worldUpType="objectrotation", worldUpVector=(0,0,1), worldUpObject="limb_C0_mid_ctl")
