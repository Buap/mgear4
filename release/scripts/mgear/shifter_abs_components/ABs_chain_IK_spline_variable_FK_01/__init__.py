...
from mgear.core import attribute, node, vector
...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        for i, t in enumerate(transform.getChainTransform2(self.guide.apos, self.normal, self.negate, axis=axis_ori)):

            ik_npo = primitive.addTransform( self.root, self.getName("ik%s_npo" % i), t)
        ...
        applyop.curvecns_op(self.mst_crv,self.ik_ctl)
        self.slv_crv=curve.addCurve(self.setup, self.getName("slv_crv"), [datatypes.Vector()] * 10)

        self.mst_crv.setAttr("visibility", False)
        self.slv_crv.setAttr("visibility", False)

        self.spline_jnts_pos = []
        # self.preiviousCtlTag = self.parentCtlTag
        for i in range(self.settings["fkNb"]):
            blend = i*(1/(self.settings["fkNb"] - 1))
            self.spline_jnts_pos.append(vector.linearlyInterpolate(self.guide.apos[0], self.guide.apos[-1], blend))

        self.splineBones = primitive.add2DChain(self.root, self.getName("splineBones%s_jnt"), self.spline_jnts_pos, self.normal, False, self.WIP)

        parentdiv = self.root
        for j in self.splineBones:
            cns = primitive.addTransform(parentdiv, self.getName("%s_cns"), j.getMatrix(ws=True))
            loc = primitive.addLocator(cns, self.getName("%s_loc"), j.getMatrix(ws=True), size = 0.1)

            pm.parentConstraint(j, cns, mo=True)
            parentdiv = loc
    ...
    def addOperators(self):
        splineIkHndl, splineIkCrv = applyop.splineIK(name="splineIk", chn=self.splineBones, parent=self.root, cParent=self.setup, curve=self.mst_crv)
        self.mst_crv.worldSpace[0].connect(splineIkHndl.inCurve, f=True)
        return
