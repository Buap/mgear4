...
class Guide(guide.ComponentGuide):
    ...
    def postInit(self):
        ...
        self.save_transform = ["root","#_loc", "eff"]
    ...
    def addObjects(self):
        ...
        vTemp = transform.getOffsetPosition(self.locs[-1], [0, 1, 0])

        #self.eff = self.addLoc("eff", self.root, vTemp)
        self.eff = self.addLoc("eff", self.locs[-1], vTemp)

        self.blade = self.addBlade("blade", self.root, self.eff)

        centers = [self.root]
        centers.extend(self.locs)
        centers.append(self.eff)
