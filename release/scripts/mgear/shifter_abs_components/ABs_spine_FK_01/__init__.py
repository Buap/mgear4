...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        for i in range(self.settings["division"]):
            ...
            if i == 0:
                int_t = transform.getTransformLookingAt(self.guide.apos[0], transform.getOffsetPosition(self.root, offset=[0, 1, 0]), self.guide.blades["blade"].z * -1, "yx", self.negate)
            if i == self.settings["division"] - 1:
                int_t = transform.setMatrixPosition(self.fk_ctl[0].getMatrix(ws=True), self.guide.apos[-1])
            if i in range(self.settings["division"])[1:-1]:
                 int_t = transform.getTransformLookingAt( self.guide.apos[i], self.guide.apos[i+1], self.guide.blades["blade"].z * -1, "yx", self.negate)

            
            fk_npo = primitive.addTransform( parentctl, self.getName("fk%s_npo" % (i)), int_t)
            fk_ctl = self.addCtl(fk_npo, "fk%s_ctl" % (i), int_t, self.color_fk, "cube", w=self.size, h=self.size * .05, d=self.size, tp=self.preiviousCtlTag)
            ...
            int_t = transform.getInterpolateTransformMatrix( t, t2, blend=blend_val)
        ...
        t = self.fk_ctl[0].getMatrix(ws=True)
        self.ik0_npo = primitive.addTransform( self.fk_ctl[0], self.getName("ik0_npo"), t)
        ...
        t = self.fk_ctl[-1].getMatrix(ws=True)
        self.ik1_npo = primitive.addTransform( self.fk_ctl[-1], self.getName("ik1_npo"), t)
        ...
        # Curves -------------------------------------------
        self.mst_crv = curve.addCnsCurve( self.root, self.getName("mst_crv"), [self.ik0_ctl, self.tan0_ctl, self.tan1_ctl, self.ik1_ctl], 3)


        
        self.slv_crv = curve.addCurve(self.root, self.getName("slv_crv"), [datatypes.Vector()] * 10, False, 3)
