...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        t = transform.getTransformLookingAt(self.guide.apos[0], self.guide.apos[1], self.normal, axis=axis, negate=self.negate)

        self.ctl_npo = primitive.addTransform( self.root, self.getName("ctl_npo"), t)
        self.base_ctl = self.addCtl(self.ctl_npo, "base_ctl", t, self.color_ik, "square", w=.3, tp=self.parentCtlTag)

        attribute.setKeyableAttributes(self.base_ctl, self.tr_params)

        self.base_upv = primitive.addTransform( self.base_ctl, self.getName("base_upv"), t)
        ...
        self.tan0_npo = primitive.addTransform( self.base_ctl, self.getName("tan0_npo"), t)

        self.tan0_ctl = self.addCtl( self.tan0_npo, "tan0_ctl", t, self.color_ik, "sphere", w=.4, tp=self.base_ctl )

        attribute.setKeyableAttributes(self.tan0_ctl, self.t_params)

        self.tan0_upv = primitive.addTransform( self.tan0_ctl, self.getName("tan0_upv"), t)
        self.tan0_upv.attr("tz").set(.01)

        t = transform.getTransformLookingAt(self.guide.apos[-2], self.guide.apos[-1], self.normal, axis=axis, negate=self.negate)
        t = transform.setMatrixPosition(t, self.guide.apos[-1])
        self.ik_cns = primitive.addTransform( self.root, self.getName("ik_cns"), t)
        self.tip_npo = primitive.addTransform( self.ik_cns, self.getName("tip_npo"), t)

        self.tip_ctl = self.addCtl(self.tip_npo, "tip_ctl", t, self.color_ik, "square", w=.3, tp=self.base_ctl)
        ...
        # Curves -------------------------------------------
        #self.mst_crv = curve.addCnsCurve( self.root, self.getName("mst_crv"), [self.base_ctl, self.tan0_ctl, self.tan1_ctl, self.tip_ctl], 3)
        #self.upv_crv = curve.addCnsCurve( self.root, self.getName("upv_crv"), [self.base_upv, self.tan0_upv, self.tan1_upv, self.tip_upv], 3)
        

        self.mst_crv=curve.addCurve(self.setup, self.getName("mst_crv"), self.guide.apos)
        applyop.curvecns_op(self.mst_crv,[self.base_ctl, self.tan0_ctl, self.tan1_ctl, self.tip_ctl])
        self.upv_crv=curve.addCurve(self.setup, self.getName("upv_crv"), self.guide.apos)
        applyop.curvecns_op(self.upv_crv,[self.base_upv, self.tan0_upv, self.tan1_upv, self.tip_upv])
