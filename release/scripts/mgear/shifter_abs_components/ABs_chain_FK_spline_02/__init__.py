...
class Component(component.Main):
    ...
    def addObjects(self):
        ...
        numCVs = len(self.guide.apos)
        cvPos = []
        for i in range(numCVs):
            cvPos.append((i,0,0))

        self.abs_mst_crv=curve.addCurve(self.setup, self.getName("absMst_crv"), cvPos)
        self.abs_upv_crv=curve.addCurve(self.setup, self.getName("absUpv_crv"), cvPos)
        self.abs_mst_crv.setAttr("visibility", False)
        self.abs_upv_crv.setAttr("visibility", False)
        ...
        for i, t in enumerate(transform.getChainTransform(self.guide.apos,
            ...
            m = node.createDecomposeMatrixNode(tweak_ctl.worldMatrix[0])
            m.outputTranslate.connect(self.abs_mst_crv.controlPoints[i])
            m = node.createDecomposeMatrixNode(upv_curv_lvl.worldMatrix[0])
            m.outputTranslate.connect(self.abs_upv_crv.controlPoints[i])
        ...
        m = node.createDecomposeMatrixNode(tweak_ctl.worldMatrix[0])
        m.outputTranslate.connect(self.abs_mst_crv.controlPoints[numCVs-1])
        m = node.createDecomposeMatrixNode(upv_curv_lvl.worldMatrix[0])
        m.outputTranslate.connect(self.abs_upv_crv.controlPoints[numCVs-1])
    ...
    def addOperators(self):
        ...
        if self.settings["keepLength"]:
            arclen_node = pm.arclen(self.abs_mst_crv, ch=True)
            alAttr = pm.getAttr(arclen_node + ".arcLength")
            ration_node = node.createMulNode(self.length_ratio_att,
                                             alAttr)

            pm.addAttr(self.abs_mst_crv, ln="length_ratio", k=True, w=True)
            node.createDivNode(arclen_node.arcLength,
                               ration_node.outputX,
                               self.abs_mst_crv.length_ratio)

            div_node_scl = node.createDivNode(self.abs_mst_crv.length_ratio,
        ...
        for i in range(self.def_number):
            ...
            cns = applyop.pathCns(
                self.div_cns[i], self.abs_mst_crv, False, u, True)
