...
class Component(MainComponent):
    ...
    def addObjects(self):
        ...
        self.limbBones = pri.add2DChain(self.root, self.getName("armBones%s_jnt"), self.guide.apos[0:3], self.normal, False, self.WIP)
        self.limbBonesFK = pri.add2DChain(self.root, self.getName("armBonesFK%s_jnt"), self.guide.apos[0:3], self.normal, False, self.WIP)
        self.limbBonesIK = pri.add2DChain(self.root, self.getName("armBonesIK%s_jnt"), self.guide.apos[0:3], self.normal, False, self.WIP)


        # Fk Controls
        t = tra.getTransformLookingAt(self.guide.apos[0], self.guide.apos[1], self.normal, "xz")
        self.fk0_npo = pri.addTransform(self.root, self.getName("fk0_npo"), t)
        self.fk0_ctl = self.addCtl(self.fk0_npo, "fk0_ctl", t, self.color_fk, "circle", w=self.length0, ro=dt.Vector(0,0,1.5708), degree=3)
        att.setKeyableAttributes(self.fk0_ctl, ["tx", "ty", "tz", "ro", "rx", "ry", "rz"])

        t = tra.getTransformLookingAt(self.guide.apos[1], self.guide.apos[2], self.normal, "xz")
        self.fk1_npo = pri.addTransform(self.fk0_ctl, self.getName("fk1_npo"), t)
        self.fk1_ctl = self.addCtl(self.fk1_npo, "fk1_ctl", t, self.color_fk, "circle", w=self.length0, ro=dt.Vector(0,0,1.5708), degree=3)
        att.setKeyableAttributes(self.fk1_ctl, ["tx", "ty", "tz", "ro", "rx", "ry", "rz"])

        t = tra.getTransformLookingAt(self.guide.apos[2], self.guide.apos[3], self.normal, "xz")
        self.fk2_npo = pri.addTransform(self.fk1_ctl, self.getName("fk2_npo"), t)
        self.fk2_ctl = self.addCtl(self.fk2_npo, "fk2_ctl", t, self.color_fk, "circle", w=self.length0, ro=dt.Vector(0,0,1.5708), degree=3)
        ...
        self.upv_npo = pri.addTransformFromPos(self.root, self.getName("upv_npo"), v)
        self.upv_ctl = self.addCtl(self.upv_npo, "upv_ctl", tra.getTransform(self.upv_npo), self.color_ik, "diamond", w=self.size*.12)


        self.ik_npo = pri.addTransformFromPos(self.root, self.getName("ik_npo"), self.guide.pos["ankle"])

        m = tra.getTransformFromPos(self.guide.pos["ankle"])
        self.ik_ctl = self.addCtl(self.ik_npo, "ik_ctl", m, self.color_ik, "cube", w=self.size*.12, h=self.size*.12, d=self.size*.12)
    ...
    def addOperators(self):
        ...
        for i,limb in enumerate(self.limbBones):
            con = pm.parentConstraint(self.limbBonesFK[i], self.limbBonesIK[i], limb)
            ...
            # attach fk controls
            pm.parentConstraint(self.fk_ctl[i], self.limbBonesFK[i])
        
        self.ikHandle = pri.addIkHandle(self.ik_ctl, self.getName("ik2BonesHandle"), self.limbBonesIK, self.ikSolver, self.upv_ctl)
